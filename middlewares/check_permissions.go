...
func CheckPermissions(db databases.Database, permission permissions.Permission) func(next echo.HandlerFunc) echo.HandlerFunc {
	return func(next echo.HandlerFunc) echo.HandlerFunc {
		return func(ctx echo.Context) error {
			...
			// Log information
			if hasPermission {
				log.Printf("User with ID %d has the required permission: %s", userID, permission)
			} else {
				log.Printf("User with ID %d does not have the required permission: %s", userID, permission)
			}
